		...
		try {
			...
			if (cursor != null && cursor.getCount() > 0) {
				...
				//	int dIndex = cursor
					...
					String word =WQFerhengQueryProvider.GetValue(cursor, column); //cursor.getString(dIndex);
					if(filterlower.equalsIgnoreCase("")||word.toLowerCase().startsWith(filterlower))
					{
						...
						String w_n = WQFerhengQueryProvider.GetValue(cursor, WQFerhengDB.KEY_WORD_N);
						...
						if(wordd==null||wordd.equalsIgnoreCase("")	)
						{
							...
							item.put(WQFerhengDB.KEY_WORD, wordd);
						item.put(WQFerhengDB.KEY_DEFINITION, def);
						Words w=new Words();
						w.id=id;
						w.peyv=wordd;		
