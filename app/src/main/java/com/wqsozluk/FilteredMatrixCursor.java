...
public class FilteredMatrixCursor extends CursorWrapper {
	...
	private void Filter(Cursor cursor, String filter, String column) {
		try {
			...
			matrixCursor=new MatrixCursor(new String[]{BaseColumns._ID, WQDictionaryDB.KEY_WORD,WQDictionaryDB.KEY_WORD_N, WQDictionaryDB.KEY_DEFINITION});
			...
			if (cursor != null && cursor.getCount() > 0) {
				...
				{				
					String word_n =WQDictionaryQueryProvider.GetValue(cursor, WQDictionaryDB.KEY_WORD_N);
					...
					{				
						...
						String id = WQDictionaryQueryProvider.GetValue(cursor, BaseColumns._ID);
			...
			for(int i=0;i<Items.size(); i++)
			{
				...
				item.put(WQDictionaryDB.KEY_WORD, w.peyv);
				item.put(WQDictionaryDB.KEY_DEFINITION, w.wate);
