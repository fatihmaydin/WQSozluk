		...
		try {
			...
			if (cursor != null && cursor.getCount() > 0) {
				...
				while (!cursor.isAfterLast()) {
					//int dIndex = cursor
						//	.getColumnIndexOrThrow(WQDictionaryDB.KEY_DEFINITION);
					String ddef = WQDictionaryQueryProvider.GetValue(cursor, WQDictionaryDB.KEY_DEFINITION ) ;//cursor.getString(dIndex);
					//int dIndexw = cursor
						//	.getColumnIndexOrThrow(WQDictionaryDB.KEY_WORD);
					String wword = WQDictionaryQueryProvider.GetValue(cursor, WQDictionaryDB.KEY_WORD );//cursor.getString(dIndexw);
					if(wword==null||wword.equalsIgnoreCase(""))
					{
						//int dIndexw_n = cursor
							//	.getColumnIndexOrThrow(WQDictionaryDB.KEY_WORD_N);
						String wword_n =WQDictionaryQueryProvider.GetValue(cursor, WQDictionaryDB.KEY_WORD_N ); //cursor.getString(dIndexw_n);
						wword=wword_n;
					}
	...
	private void Filter(Cursor cursor, String filter) {
		try {
			...
			if (cursor != null && cursor.getCount() > 0) {
				...
				//	int dIndex = cursor
					//		.getColumnIndexOrThrow(WQDictionaryDB.KEY_DEFINITION);
					String word =WQDictionaryQueryProvider.GetValue(cursor, WQDictionaryDB.KEY_DEFINITION ); //cursor.getString(dIndex);
					//int dIndexw = cursor
						//	.getColumnIndexOrThrow(WQDictionaryDB.KEY_WORD);
					String def = WQDictionaryQueryProvider.GetValue(cursor, WQDictionaryDB.KEY_WORD );//cursor.getString(dIndexw);
