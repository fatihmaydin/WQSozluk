...
public class ListViewGotinAdapter extends BaseAdapter implements OnTouchListener {
	...
	public ListViewGotinAdapter(Context context, String text) 
	{
		...
		if (lines != null && lines.length > 0
				&&( lines[0].toLowerCase().trim().startsWith((String) myBaseContext.getText(R.string.Gotin))
						||lines[0].toLowerCase().trim().startsWith((String) myBaseContext.getText(R.string.Xwendin))
						||lines[0].toLowerCase().trim().startsWith((String) myBaseContext.getText(R.string.bilevkirin))))
		{
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		//Toast.makeText(myBaseContext, item, Toast.LENGTH_LONG).show();
			...
			button.setFocusable(false);
				...
				public void onClick(View v) 
				{
							...
							)
					{
						...
						else if(uniqlineksuffix.contains(".wav"))
							uniqlineksuffix=uniqlineksuffix.substring(0, uniqlineksuffix.indexOf(".wav")+4);
					}
					...
					Log.d("newlinkuniqlineksuffix2",newlinkuniqlineksuffix );
					try {
