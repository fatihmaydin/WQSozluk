...
public class ExpandableListAdapter extends BaseExpandableListAdapter {
	...
	WQDictionaryActivity wFerhengActivity;
	...
	public ExpandableListAdapter(WQDictionaryActivity pContext,
		...
		wFerhengActivity = (WQDictionaryActivity) pContext;
	...
	public Boolean IsListContaining(List<String> list, String text)
	{
		if(list==null||text.equalsIgnoreCase(""))
		{
			return false;
		}
		//Toast.makeText(mContext,text+list,Toast.LENGTH_LONG ).show();;
		...
		for (String s:list)
		{
			...
			//Toast.makeText(wFerhengActivity,text+" "+s,Toast.LENGTH_LONG ).show();
			if(s.toLowerCase().equalsIgnoreCase(text.toLowerCase().replace(":",""))||
					text.toLowerCase().equalsIgnoreCase(s.toLowerCase().replace(":","")))
			{
				bContains=true;
				break;
			}
	...
	private Boolean IsListContainingStartWit(List<String> list, String text)
	{
		if(list==null||text.equalsIgnoreCase(""))
		{
			return false;
		...
		for (String s:list)
		{
//			if(text.toLowerCase().startsWith("alman"))
//			{
//				Toast.makeText(mContext, text+" translation "+bContains+"  "+s, Toast.LENGTH_LONG).show();
//			}
			if(text==null||text.equalsIgnoreCase(""))
			{
				continue;
			}
			if(s.toLowerCase().startsWith(text.toLowerCase())||text.toLowerCase().startsWith(s.toLowerCase()))
			{
				//Toast.makeText(mContext, "if(s.toLowerCase().startsWith(text.toLowerCase()))", Toast.LENGTH_LONG).show();
				bContains=true;
				break;
			}
		}
	...
	public View getChildView(int arg0, int arg1, boolean arg2, View arg3,
		...
		{
			text=regexMatcherberalreplace.replaceAll(regexMatcherberalreplace.group().replace(". :", ". ")
					.replace(".:", ". "));
		}
		...
		if(WQDictionaryActivity.ShowHeaderTranslation) {
			if (WQDictionaryActivity.mapLanguages.containsKey(line0.replace(":", ""))) {
...
//					wFerhengActivity.makeText(line0.replace(":", ""));
		...
		text=newtext;
//		Toast.makeText(mContext,mContext.getString( R.string.bilevkirin), Toast.LENGTH_LONG).show();
		if (lines != null && lines.length > 0
				&& (lines[0].toLowerCase().trim().equalsIgnoreCase(mContext.getString( R.string.Gotin))||lines[0].toLowerCase().trim().equalsIgnoreCase(mContext.getString( R.string.Xwendin))
						||lines[0].toLowerCase().trim().equalsIgnoreCase(mContext.getString( R.string.bilevkirin))
		...
		if (wFerhengActivity != null) {
											...
											// linkifying
			{
				...
				if ((aftersection.toLowerCase().startsWith("Alt kavramlar") || aftersection
						.startsWith("واتە")||aftersection.toLowerCase().startsWith((String) mContext.getText(R.string.Xwendin))||
						aftersection.toLowerCase().startsWith((String) mContext.getText(R.string.Gotin))||
						aftersection.toLowerCase().startsWith((String) mContext.getText(R.string.bilevkirin))&&!lines[0].contains(":"))) {
					int offset = 0;
					...
					for (int xx = 0; xx < lines.length; xx++)
					{
						String line = lines[xx];
						Boolean isCat=true;
						if(WQDictionaryActivity.wiki.equalsIgnoreCase("de"))
							isCat=false;
						else {
		...
		//Toast.makeText(mContext, line0+WQDictionaryActivity.listlinkedSections, Toast.LENGTH_LONG).show();
		//if (lines[0].toLowerCase().startsWith("wate")||lines[0].toLowerCase().startsWith("واتە"))
		//Toast.makeText(mContext, line0+"found", Toast.LENGTH_LONG).show();

		if(IsListContaining(WQDictionaryActivity.listlinkedSections,line0 ))
//		if ( WQDictionaryActivity.listlinkedSections.stream()
//				.anyMatch(item -> item.equalsIgnoreCase(line0.toLowerCase())))
		{
		...
		else if (line0.startsWith((String) mContext.getText(R.string.Gotin))) {

			tv.setText(span);
			return arg3;

		}	
		else if (line0.toLowerCase().startsWith("ji wêjeya klasîk"))
		{		
		...
		//else if (lines[0].toLowerCase().startsWith((String) mContext.getText( R.string.strtranslation)))
//		else if ( WQDictionaryActivity.headerTranslations.stream()
//				.anyMatch(item -> item.equalsIgnoreCase(line0)))

		else if((IsListContaining(WQDictionaryActivity.headerTranslations,line0 )))
		{
//			if(line0.toLowerCase().startsWith("çeviri"))
//			Toast.makeText(mContext, line0+" translation", Toast.LENGTH_LONG).show();
			for (int xx = 1; xx < lines.length; xx++) {

				String line = lines[xx];
//				if(line0.toLowerCase().startsWith("çeviri"))
//				Toast.makeText(mContext, line0+" translation "+line+"  "+WQDictionaryActivity.listTranslatedLangs, Toast.LENGTH_LONG).show();
				if(	IsListContainingStartWit(WQDictionaryActivity.listTranslatedLangs,line ))
//					if(line0.toLowerCase().startsWith("çeviri"))
//						Toast.makeText(mContext, line0+" translation "+line+"  "+WQDictionaryActivity.listTranslatedLangs, Toast.LENGTH_LONG).show();
//				if ( WQDictionaryActivity.listTranslatedLangs.stream()
...
//						|| line.toLowerCase().contains("almanî"))
		...
		} else if(	IsListContaining(WQDictionaryActivity.headerList,line0 )
			...
			//Toast.makeText(mContext,  strsub.substring(0,1)+" translation", Toast.LENGTH_LONG).show();
						if(strsub.startsWith("•"))
						{
							MakeItalic(span, lines, text, strsub.replace("Þ"," "), offset);
						}
						offset+=str[x].length()+1;
...
//				|| lines[lines.length - 1].startsWith("binihêre")
		IsListContaining(WQDictionaryActivity.listBinihere,lines[lines.length - 1].trim()
		)) {
			...
			if (IsSpannable(strsubspan)) {
				...
				IsListContaining(WQDictionaryActivity.listBinihere,	lines[lines.length - 2]
...
//				parentName.contains("Binihêre")
//				|| parentName.contains("Binere")
//				|| parentName.contains("بنهێرە")
		IsListContaining(WQDictionaryActivity.listBinihere,	parentName
		)) {
...
//			if (lines[0].toLowerCase().startsWith((String) mContext.getText(R.string.werger)))
		...
		//	wFerhengActivity.makeText(line);
			...
			for (int x = startIndexofLine; x < str.length; x++) {
				...
				if(lines[0].toLowerCase().startsWith(mContext.getString(R.string.Wate))&&
						IsListContaining(WQDictionaryActivity.listBinihere,	strsubspan.toLowerCase().trim()
						//strsubspan.toLowerCase().trim().startsWith((String) mContext.getText(R.string.strbinihere))
				)) //linkfy binihêre underneath wate
				{
				...
				if(x>0&&IsListContaining(WQDictionaryActivity.listBinihere,str[x-1].trim().toLowerCase() )
						//str[x-1].trim().toLowerCase().contains(mContext.getText(R.string.strbinihere))
				)
				{
		...
		if (strsubx.length > 1 && strsubx.length <= 3) {
			...
			if (strsubx.length == 2) {
				...
				if (tocontrol2.equalsIgnoreCase("ne")
						...
						|| tocontrol2
								.equalsIgnoreCase((String) mContext.getText(R.string.strbinihere)))
					isspannable = true;
			}
...
//						definitionActivity.makeText("aaa"+WQDictionaryActivity.ShowCategoryDialog);
					Log.d("clc", "cj,"+WQDictionaryActivity.ShowCategoryDialog);
					
					try {
						if(WQDictionaryActivity.ShowCategoryDialog)
						{
						...
						builder.setPositiveButton((String) mContext.getText(R.string.dialogEre),
...
//											definitionActivity.makeText("aaa"+word);
						...
						builder.setNegativeButton((String) mContext.getText(R.string.dialogNa),
								new DialogInterface.OnClickListener() {
						...
						builder.setNeutralButton((String) mContext.getText(R.string.dialogNeprise),
								new DialogInterface.OnClickListener() {

									@Override
									public void onClick(DialogInterface dialog, int which) {
										
										SharedPreferences sharedPref =mContext. getSharedPreferences(WQDictionaryActivity.PRIVATE_PREF,
												Context.MODE_PRIVATE);
										Editor editor = sharedPref.edit();
										WQDictionaryActivity.ShowCategoryDialog=false;
			...
			//if (!isCat)
			span.setSpan(new
							ForegroundColorSpan(Color.parseColor(WQDictionaryActivity. linkColor)),
					start,end,Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
	...
	public View getGroupView(int arg0, boolean arg1, View arg2, ViewGroup arg3) {
		...
		if(WQDictionaryActivity.ShowLanguageTranslation) {
	...
	{
		String translation="";
		if(WQDictionaryActivity.mapLanguages!=null&&WQDictionaryActivity.mapLanguages.containsKey(name)&&WQDictionaryActivity.LocalisationIndex>-1)
		{
			List listtranslation=WQDictionaryActivity.mapLanguages.get(name);
			translation=listtranslation.get(WQDictionaryActivity.LocalisationIndex).toString();
		}
